...
<body>
  ...
  <script>
    ...
    async function checkShelf() {{
      ...
      resultsDiv.innerHTML = '<p style="text-align:center; padding:40px;">Analisi in corso...</p>';
      ...
      try {{
        ...
        resultsDiv.innerHTML = '<p style="color:red; text-align:center;">Errore di connessione al server</p>';
      }}
    }}
    ...
    async function showAllProducts() {{
      const shelfId = document.getElementById('shelfSelect').value;
      const resultsDiv = document.getElementById('results');
      resultsDiv.innerHTML = '<p style="text-align:center; padding:40px;">Caricamento di tutti i prodotti...</p>';
    ...
    function renderResults(data) {{
      ...
      if (data.safe_products && data.safe_products.length > 0) {{
        ...
        html += `<div class="no-results">Nessun prodotto compatibile trovato con i filtri selezionati.</div>`;
      }}
      div.innerHTML = html;
    }}
